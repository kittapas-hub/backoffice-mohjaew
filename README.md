...
(`confirm_slip_payment`, migration `0010`) → booking เปลี่ยน
...
- ทดสอบ end-to-end บน staging: รัน `0010` ใน SQL Editor, ตั้ง env ครบ,
  จองคิว → เปิดหน้า success → "อัปโหลดสลิป" → ใช้สลิปโอนจริงเข้าบัญชีร้าน
  (ยอดตรง) → booking ต้องเป็น `confirmed` และมีแถว outbox + audit
- ไม่มี sandbox จาก EasySlip: ใช้สลิปจริงยอดเล็กบน staging เท่านั้น

### Rollout → production

1. Merge หลังผ่าน review อิสระเท่านั้น
2. รัน `supabase/migrations/0010_slip_verification.sql` ใน SQL Editor (ตาม
   ขั้นตอน manual migration เดิมของโปรเจกต์)
3. ตั้ง env ทั้ง 3 ตัวบน Vercel Production (`SLIP_RECEIVER_ACCOUNTS` ให้เก็บ
   ค่าจากการ verify สลิปจริงก่อนหนึ่งใบผ่าน staging)
4. Deploy แล้วทดสอบด้วยการจอง + โอนจริงยอดเล็ก 1 รายการ
...
- ถอน DB objects: `drop function public.confirm_slip_payment(...)` (ดูท้าย
  ไฟล์ 0010) — เก็บตาราง `payment_slip_verifications` ไว้เป็นหลักฐาน audit
