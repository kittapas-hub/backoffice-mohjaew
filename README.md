...
| `LINE_BOOKING_GROUP_ID` | group ID ของกลุ่มทีมงานที่จะรับแจ้งเตือน |
...
3. ใส่ค่า `Cxxxx...` ใน `LINE_BOOKING_GROUP_ID`

> **ชื่อตัวแปรเดียว ไม่มี fallback**: โค้ดอ่านเฉพาะ `LINE_BOOKING_GROUP_ID` เท่านั้น — ไม่มี
> dual-name support และไม่มี fallback ไปยังชื่อตัวแปรอื่นใด หากมีการรีเนมตัวแปรนี้บน Vercel
> ในอดีต ให้เพิ่มค่าใหม่ก่อน, deploy แล้วยืนยันว่าแจ้งเตือนยังส่งถึงกลุ่มทีมงานได้ปกติ, แล้วจึงค่อยลบ
> ตัวแปรชื่อเดิมออกจาก Vercel เป็นลำดับสุดท้าย
...
- **ถ้ายังไม่ตั้ง `LINE_BOOKING_GROUP_ID` หรือ token → ไม่ทำให้ booking ล้มเหลว**
...
## 12. Automatic slip verification (Phase 1 — EasySlip)

ลูกค้าโอน PromptPay แล้วอัปโหลดสลิปที่ `/pay/[token]` → เซิร์ฟเวอร์ตรวจไฟล์จริง
(magic bytes + ขนาด + dimension) → ส่งให้ EasySlip ตรวจ → ตรวจนโยบาย (ยอด,
บัญชีผู้รับ, เวลาโอน, เลขอ้างอิงธุรกรรม) → ยืนยันด้วย RPC เดียวแบบ atomic
(`confirm_slip_payment`, migration `0011`) → booking เปลี่ยน
`pending_payment → confirmed` (สถานะเดียวกับปุ่มยืนยันของแอดมิน) + สร้างแถว
notification outbox. **ปุ่มยืนยันแบบ manual ของแอดมินยังอยู่ครบ** ใช้เป็น
fallback สำหรับทุกเคสที่ระบบอัตโนมัติไม่ยืนยันให้

### Environment variables (วางค่าจริงบน Vercel เท่านั้น — ห้าม commit)
...
- ไฟล์: JPG / PNG / WebP เท่านั้น, ≤ 4 MB, 100–8000 px ต่อด้าน (ต่ำกว่าเพดานคำขอรวม 4.5 MB ของ Vercel Functions),
...
- ชุดทดสอบอัตโนมัติ mock ทั้ง fetch ของ EasySlip และไม่แตะ API จริง:
  `npm run test` (ไฟล์ `easyslip.test.ts`, `slip-policy.test.ts`,
  `slip-confirm.test.ts`, `image-meta.test.ts`)
- ก่อนทดสอบ slip verification ให้รัน `0010_reconcile_0006_0009.sql` เฉพาะ
  environment ที่ผ่านรายงาน baseline และได้รับอนุมัติ แล้วตรวจผลด้วย SQL
  verification แบบ read-only ก่อน
- ทดสอบ end-to-end slip verification: รัน `0011_slip_verification.sql`, ตั้ง env
  ครบ, จองคิว → เปิดหน้า success → "อัปโหลดสลิป" → ใช้สลิปโอนจริงเข้าบัญชีร้าน
  (ยอดตรง) → booking ต้องเป็น `confirmed` และมีแถว outbox + audit
- ไม่มี sandbox จาก EasySlip: ใช้สลิปจริงยอดเล็กใน environment ที่ได้รับอนุมัติเท่านั้น
...
ระบบจะ **ไม่มีวัน** ยืนยันอัตโนมัติเมื่อ: ยอดไม่ตรง, บัญชีผู้รับไม่ตรง,
เวลาโอนอยู่นอกช่วง, อ่านเลขอ้างอิงไม่ได้, สลิปซ้ำ, hold หมดอายุ, booking
expired/cancelled, หรือ EasySlip ล่ม/ตอบผิดรูป เคสเหล่านี้:

1. ลูกค้าเห็นข้อความชัดเจนให้ติดต่อทีมงานทาง LINE พร้อมสลิป
2. เคส "เงินเข้าจริงแต่ booking ไม่ eligible" → order ถูกตั้งเป็น
   `manual_review` + ทีมได้รับแจ้งผ่าน outbox (`slip_manual_review`)
3. ทีมตรวจใน `/admin/bookings/[id]` แล้วใช้ปุ่มยืนยัน manual เดิม (สำหรับ
   pending_payment ที่ hold ยังไม่หมด) หรือให้ลูกค้าจองใหม่/คืนเงินตามนโยบาย
4. ตรวจประวัติความพยายามทั้งหมดได้จากตาราง `payment_slip_verifications`
