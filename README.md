...
**รอบเวลา** (เช่น 09:00–12:00) ที่มี capacity ต่อรอบ → สร้าง booking สถานะ `pending_payment`
และ **hold คิว 60 นาที** → ถ้าไม่ชำระจะ `expired` และคืนที่ว่าง → `/admin/day` จัดการรายวัน
...
2. เลือกวัน → เห็นเฉพาะรอบที่ **เปิด** และ **ยังมีที่เหลือ** พร้อมจำนวนคิวคงเหลือ
3. เลือกรอบ + กรอกข้อมูล (ชื่อเล่น, เบอร์, วันเกิด, หัวข้อ)
4. กดยืนยัน → สร้าง booking สถานะ `pending_payment` + hold 60 นาที + ได้ **ลำดับคิวในรอบ**
...
- เลือกวัน → เห็นแต่ละรอบ (เช้า/บ่าย/เย็น), จำนวนจอง/capacity, รายชื่อลูกค้าในรอบ
- ปุ่ม **สร้างรอบมาตรฐาน** (ถ้ายังไม่มีรอบของวันนั้น), **แก้ capacity รายวัน**, **ปิด/เปิดรอบ**
...
- [ ] `/admin/day` → "สร้างรอบมาตรฐาน" → เห็น 4 รอบ; ตั้ง capacity รอบหนึ่ง = 4
- [ ] จองจาก `/booking?source=website` ครบ 4 (เบอร์ต่างกัน) → คิว 1–4; ครั้งที่ 5 → 409 `slot_full`
- [ ] กด "ปิดรอบ" → `/booking` ไม่แสดงรอบนั้น และ API ตอบ 409 `slot_closed`
- [ ] ปล่อย `pending_payment` หมด 60 นาที (หรือยิง cron) → `expired` และที่ว่างกลับมา
